...
def populate_stats():
    ...
    response_car = requests.get(app_config['eventstore']['url1'] + "/CarEvent?start_timestamp=" + json_object['last_updated'] +"&end_timestamp=" + current_datetime)
    response_bike = requests.get(app_config['eventstore']['url2'] + "/BikeEvent?start_timestamp=" + json_object['last_updated'] + "&end_timestamp=" + current_datetime)
